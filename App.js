    ...
    <SafeAreaView style={s.safe}>
      ...
      <FlatList data={visibleItems} keyExtractor={(item) => item.id} contentContainerStyle={s.container} showsVerticalScrollIndicator={false}
        ListHeaderComponent={<>
          ...
          <View style={s.categoryHeader}><Text style={s.categoryLabel}>دسته‌بندی</Text><Pressable onPress={() => setCategoryOpen(true)}><Text style={s.addCategoryText}>＋ افزودن</Text></Pressable></View>
        ...
        ListFooterComponent={<View style={s.footer}><Pressable onPress={shareList}><Text style={s.footerLink}>اشتراک‌گذاری لیست ↗</Text></Pressable><Text style={s.footerText}>خریدیار</Text></View>}
      />
      ...
      <Modal visible={menuOpen} transparent animationType="slide" onRequestClose={() => setMenuOpen(false)}><View style={s.backdrop}><Pressable style={s.dismiss} onPress={() => setMenuOpen(false)} /><View style={s.sheet}><View style={s.handle} /><Text style={s.sheetTitle}>منوی خریدیار</Text><Pressable style={s.menuRow} onPress={() => { setMenuOpen(false); setCategoryOpen(true); }}><Text style={s.menuText}>⚙️ مدیریت دسته‌بندی‌ها</Text><Text style={s.menuArrow}>‹</Text></Pressable><Pressable style={s.menuRow} onPress={() => { setMenuOpen(false); setHistoryOpen(true); }}><Text style={s.menuText}>🗂️ لیست‌های خرید قبلی</Text><Text style={s.menuArrow}>‹</Text></Pressable><Pressable style={s.menuRow} onPress={archiveList}><Text style={s.menuText}>📦 آرشیو لیست فعلی</Text><Text style={s.menuArrow}>‹</Text></Pressable></View></View></Modal>
