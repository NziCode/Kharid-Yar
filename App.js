...
import { Menu, Plus, Search, Settings2, History, Share2, Trash2, Check, ChevronDown, FolderPlus, UserRound } from 'lucide-react';
import {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <FlatList data={visibleItems} keyExtractor={(item) => item.id} contentContainerStyle={s.container} showsVerticalScrollIndicator={false}
        ListHeaderComponent={<>
            <View style={s.header}><View style={s.headerCopy}><Text style={s.greeting}>سلام {userName} 👋</Text><Text style={s.subtitle}>امروز چه چیزی لازم داری؟</Text></View><View style={s.headerActions}><Pressable onPress={() => setMenuOpen(true)} style={s.icon}><Menu size={21} color="#6554E8" /></Pressable><View style={s.avatar}><Text style={s.avatarText}>{userName.slice(0, 1)}</Text></View></View></View>
          ...
          <View style={s.searchBox}><Search size={19} color="#6554E8" /><TextInput value={search} onChangeText={setSearch} placeholder="جست‌وجوی کالا..." placeholderTextColor="#A2A6B4" style={s.searchInput} /></View>
          ...
          <View style={s.categoryHeader}><Text style={s.categoryLabel}>دسته‌بندی</Text><Pressable onPress={() => setCategoryOpen(true)} style={s.addCategoryAction}><Plus size={15} color="#6554E8" /><Text style={s.addCategoryText}>افزودن</Text></Pressable></View>
        ...
        ListFooterComponent={<View style={s.footer}><Pressable onPress={shareList} style={s.footerAction}><Share2 size={14} color="#6554E8" /><Text style={s.footerLink}>اشتراک‌گذاری لیست</Text></Pressable><Text style={s.footerText}>خریدیار</Text></View>}
      />
      <Pressable onPress={() => setAddOpen(true)} style={s.fab}><Plus size={21} color="#FFF" /><Text style={s.fabText}>افزودن کالا</Text></Pressable>
      ...
      <Modal visible={menuOpen} transparent animationType="slide" onRequestClose={() => setMenuOpen(false)}><View style={s.backdrop}><Pressable style={s.dismiss} onPress={() => setMenuOpen(false)} /><View style={s.sheet}><View style={s.handle} /><Text style={s.sheetTitle}>منوی خریدیار</Text><Pressable style={s.menuRow} onPress={() => { setMenuOpen(false); setCategoryOpen(true); }}><Settings2 size={18} color="#6554E8" /><Text style={s.menuText}>مدیریت دسته‌بندی‌ها</Text></Pressable><Pressable style={s.menuRow} onPress={() => { setMenuOpen(false); setHistoryOpen(true); }}><History size={18} color="#6554E8" /><Text style={s.menuText}>لیست‌های خرید قبلی</Text></Pressable><Pressable style={s.menuRow} onPress={archiveList}><FolderPlus size={18} color="#6554E8" /><Text style={s.menuText}>آرشیو لیست فعلی</Text></Pressable></View></View></Modal>
